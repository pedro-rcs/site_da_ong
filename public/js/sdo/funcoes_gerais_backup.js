/* funcoes_index_institucional - Início */

// const servidor = 'https://192.168.0.176:3001'
...
function foca_input (qual, acao) {
  ...
  if (acao === 'in') {
    ...
    document.getElementById('span_scroll_1').style.display = 'none'
    document.getElementById('i_scroll').style.display = 'none'
    document.getElementById('navbar').style.display = 'none'
    document.getElementById('recip_creditos_img').style.display = 'none'

  }

  if (acao == 'out') {

  	document.getElementById('span_scroll_1').style.display = 'flex'
    document.getElementById('i_scroll').style.display = 'flex'
    document.getElementById('navbar').style.display = 'flex'
    document.getElementById('recip_creditos_img').style.display = 'flex'
...
function menu_drop () {
	...
	document.getElementById('inicio').style.webkitFilter = 'grayScale(1) blur(3px)'
...
function some_menu_drop () {
	...
	document.getElementById('inicio').style.webkitFilter = 'grayScale(0) blur(0px)'
...
// Função abaixo só funciona na seleção de premium e grátis do construtor de sites.
function seleciona_radio (parametro) {
	let outro_parametro
	if (parametro === 'radio_gratis') outro_parametro = 'radio_premium'
	if (parametro === 'radio_premium') outro_parametro = 'radio_gratis'
	// Se não tá selecionado, seleciona.
	if (!document.getElementById(parametro).classList.contains('radio_selecionado')) {
	  document.getElementById(parametro).classList.add('radio_selecionado')
	}

	// Se o outro estava selecionado, deseleciona o outor.
	if (document.getElementById(outro_parametro).classList.contains('radio_selecionado')) {
		document.getElementById(outro_parametro).classList.remove('radio_selecionado')
	}

	document.getElementById('div_proximo_plano').innerHTML = `
		<div id="proximo_plano" class="flex_col center clicavel botao bot_laranja" style="width: 150px;" onclick="proximo('nome_ong');">Próximo</div>
	`
}
...
function valida_proximo (cont) {
	...
	if (cont === 'nome_ong') {
			...
			console.log("end_valido: " + end_valido)
...
function deletaFoto (parametro1, nomeFoto, posicao) {
	...
	if (posicao == capaAlbum) capaAlbum = 0
	if (posicao < capaAlbum) capaAlbum--
...
function setaVideo(idVideo, tituloVideo) {
  ...
  let contThumb = document.createElement('div')
      ...
      xizinho.className = 'xizinho fas fa-times-circle clicavel'
...
function sendForm (parametro, outroParametro) {
  ...
  // DELETA - Apagamos uma imagem
  if (parametro == 'deleta') {
    console.log("clicou no x para apagar uma img")
...
function troca_senha (acao) {
	...
	if (acao === 'vai') {
		...
		document.getElementById('recipiente_login_desliza').style.display = 'none'
	...
	if (acao === 'volta') {
		...
		document.getElementById('recipiente_login_desliza').style.display = 'flex'
	}
}

function pix_copia () {

  document.getElementById('input_payload_escondido').value = payload_atual

  navigator.clipboard.writeText(payload_atual).then( () => {
...
// Função ainda não está 100% pronta. Da uns erros no console e pahs.
function mostra_todos_evts (qtd_eventos) {
  for (let i = 0; i < qtd_eventos; i++) {
    document.getElementById(`miniatura_evento_${i}`).style.display = 'flex'
  }
}
...
function visualizar_galeria (i) {
  ...
  for (let j = 0; j < qtd_fotos; j++) {
  	...
  	galeria_mobile = galeria_mobile + `
  		<div id="galeria_${i}_foto_${j}" class="flex_row T1 exclusivo_mobile" style="will-change: transform; min-width: 100%; height: 100%; background-image: url('${elem_a_mostrar}'); background-position: center; background-repeat: no-repeat; background-size: cover;" ontouchmove="deslizou(${i}, ${j})" ontouchend="soltou(${i}, ${j})">
	 		</div>
  	`
  	*/
  ...
  document.getElementById('recipiente_lightbox').innerHTML = `
  	...
  	<div class="flex_row T1 largura_interna" style="height: 50px; margin-bottom: 5px;">
			...
			<div id="div_numeracao" class="flex_row" style="align-items: flex-end; font-size: 20pt; min-width: 100px;">
				<span id="span_id_galeria">${i_inicial}</span> / ${qtd_elementos}
			</div>
...
function gera_qr_estatico (quantia) {
  ...
  const obj_payload = {
		pixKey: glob_site_2.chave_pix,
		amount: valor_formatado,
		description: 'descripcion',
		merchantName: glob_site_2.nome_pix, // Títular da conta
		merchantCity: glob_site_2.cidade,
		txid_estatico: '***'
	}
